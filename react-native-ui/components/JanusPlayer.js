      ...
      <div className="janus-video-container">
        <div className="janus-video-status">
          {status === "Ready" && (
            <span style={{ color: "grey" }}>
              {readyText ? readyText : "Ready"}
            </span>
          )}
          {status === "Paused" && (
            <span style={{ color: "grey" }}>
              {pausedText ? readyText : "Paused"}
            </span>
          )}
          {status === "Live" && (
            <span style={{ color: "green" }}>
              {liveText ? readyText : "Live"}
            </span>
          )}
          {status === "Error" && (
            <span style={{ color: "red" }}>
              {errorText ? readyText : "Error"}
            </span>
          )}
        </div>
        <video
        ...
        {isPublisher && (
          <div className="janus-video-controls">
            {status === "Paused" && (
              <div className="janus-btn" onClick={onStart}>
                Start Recording
              </div>
            )}
            {status === "Live" && (
              <div className="janus-btn" onClick={onStop}>
                Stop Recording
              </div>
            )}
            ...
            {status === "Live" && (
              <React.Fragment>
                {isMuted && (
                  <div className="janus-btn" onClick={onUnmute}>
                    UnMute
                  </div>
                )}
                {!isMuted && (
                  <div className="janus-btn" onClick={onMute}>
                    Mute
                  </div>
                )}
              </React.Fragment>
