...
const linePathFuncs = {
  ...
  removePointsFromFront(n = 1) {
    this.removedLength = this.removedLength || 0;
    if (this.removedLength === this.smoothedPoints.length) {
      return false;
    } else {
      this.removedLength =
        this.removedLength + n > this.smoothedPoints.length
          ? this.smoothedPoints.length
          : this.removedLength + n;
      return true;
    }
  },
