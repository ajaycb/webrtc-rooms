...
const _drawRawLinePath = function (ctx, points, close, lineCap) {
  ...
  ctx.moveTo(points[0].x + delta, points[0].y + delta);

  for (let i = 1; i < points.length; i = i + 1) {
    let point = points[i];
    ctx.lineTo(point.x + delta, point.y + delta);
  }
...
const drawLinePathLatest = function (ctx, bufferCtx, shape) {
  if (shape.removedLength) {
    let pointsToDraw = shape.smoothedPoints.slice(shape.removedLength);

    bufferCtx.clearRect(0, 0, bufferCtx.canvas.width, bufferCtx.canvas.height);
    if (pointsToDraw.length > 0) {
      _drawRawLinePath(bufferCtx, pointsToDraw);
      bufferCtx.stroke();
    }
  } else {
    if (shape.tail) {
